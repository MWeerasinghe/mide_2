  ...
  useEffect(() => {
    const fetchAttendanceReport = async () => {
      try {
        ...
        const earier1 = dayjs().subtract(2, 'day').format('M/D/YYYY');
        const earier2 = dayjs().subtract(3, 'day').format('M/D/YYYY');
        const earier3 = dayjs().subtract(4, 'day').format('M/D/YYYY');
        const earier4 = dayjs().subtract(5, 'day').format('M/D/YYYY');
        const earier5 = dayjs().subtract(5, 'day').format('M/D/YYYY');
        ...
        const reportData = [
          ...
          calculateAttendance(earier1),
          calculateAttendance(earier2),
          calculateAttendance(earier3),
          calculateAttendance(earier4),
          calculateAttendance(earier5),
        ];
